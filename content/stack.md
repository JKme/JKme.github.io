...
1. 代码区：可执行指令
2. 数据区：用于存储全局变量
3. 堆区：进程可以在堆区动态的请求一定大小内存，并在用完之后归还给堆区。动态分布和回收是堆区的特点
4. 栈区：用于动态的存储函数之间的调用关系，以保证被调用函数返回时恢复到母函数中继续执行
...
![](./images/v2-9125ba203edd2bab1308ad88db2ae197_hd.jpg)
...
![](./images/v2-9ae4f530296d4a8ec7d44443e3e6d37f_hd.jpg)
...
![](./images/v2-c350a4c5e9f5bbe839606486f3100185_hd.jpg)


####0x04 被调用函数（callee）局部变量等入栈


![](./images/v2-63f07378642e6f38197ddfb817567f6c_hd.jpg)
...
![](./images/v2-4340d6c750f59519f4f87dffca03a86a_hd.jpg)


然后将基地址（ebp）内存储的调用（caller)函数的基地址从栈内弹出，并保存到ebp寄存器内。这样调用函数(caller)的ebp(基地址)信息得以恢复。此时栈顶指向返回地址。

![](./images/v2-cd8d0fef17eecfff68f29022660a1c85_hd.jpg)

####0x06  返回地址弹出

再将返回地址从栈内弹出，并存到eip寄存器内。这样调用函数（caller)的eip(指令)信息得以恢复。

![](./images/v2-3e7a4c71117d43ac59bcb3bd186a8e9f_hd.jpg)
...
![](./images/v2-e3af4f558ccb439e042e7a169cae2f97_hd.jpg)
...
在上面几个流程图里面，可以看到**参数与局部变量的分界线为EBP的值**。

![](https://pic4.zhimg.com/80/v2-89a65a58325afb516a7047b63940ee81_hd.jpg)
...
![](./images/v2-4523795fdf150416d91e4e4adc0ef0ae_hd.jpg)
...
* shellcode中不能包含空格字符(`\x10`,`\x0a`,`\x0b`,`\x0c`,`\x20`),否则shellcode会被截断。
...
![](./images/v2-a4964a02ab5a974439a157cf9d5b017c_hd.png)
...
其他两个先不记录:<https://raw.githubusercontent.com/liuxigu/liuxigu.github.io/master/1.code/2016-03-24-overflow-seh.html>
